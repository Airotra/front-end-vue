import http from '@http/http'

export const _getUserInfo = (errMsg) => http.GET('/api/auth/userInfo', {}, errMsg)
export const _userInfo = (id) => http.GET('/api/user/' + id)
export const _userInfoSave = (query) => http.PUT('/api/user/' + query.id, query)
export const _adminInfo = (id) => http.GET('/api/admin/' + id)
export const _adminInfoSave = (query) => http.PUT('/api/admin/' + query.id, query)
export const _userLogin = (query) => http.GET('/api/user/userLogin', query)
export const _userRegister = (query) => http.GET('/api/user/userRegister', query)
export const _logout = () => http.GET('/api/user/logout')
export const _userAddrInfo = (id) => http.GET('/api/user/getAddr', {id: id})
export const _userAddrInfoSave = (query) => http.PUT('/api/adress/' + query.id, query)
export const _addrDelete = (id) => http.DELETE('/api/adress/' + id)
export const _addrAdd = (query) => http.POST('/api/adress', query)
export const _userCouponInfo = (id) => http.GET('/api/user/getUserCoupon', {id: id})
export const _couponInfo = (id) => http.GET('/api/user/getCoupon', {id: id})
export const _updateCouponQuantity = (query) => http.PUT('/api/coupon/' + query.couponId, query)
export const _userRecCoupon = (query) => http.POST('/api/userCouponHas', query)
